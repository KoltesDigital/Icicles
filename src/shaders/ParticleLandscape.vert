...
void main() {
  ...
  float should = step(0.1, a_texcoord.y);
  p.y = noiseIQ(p.xyz * 16.) * 60. * should;
  // p.xyz += getOffset();
  // float size = 10. * noiseIQ(p.xyz * 0.1);
  float size = 40. + 40. * (noiseIQ(p.xyz * 32.));
  vec3 offset = vec3(a_texcoord.x * 4., a_texcoord.y * 0.1, 0) * size;
  // offset = rotateZ(offset, noiseIQ(p.xyz*12.) * 3.14159 * 2.);

  v_color = vec4(vec3(1) * (noiseIQ(p.xyz) * 0.5 + 0.5), 1);
  v_color.rgb *= a_texcoord.y;
